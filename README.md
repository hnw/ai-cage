...
設定ファイルで `autoStartCommand` を指定すると、`.ai-cage-sandboxes/<sandbox>/.vscode/tasks.json` として生成されます。フォルダを開いたタイミングで指定のコマンドが統合ターミナル上で自動起動します。
...
常に、サンドボックス側の `.vscode` ディレクトリがコンテナのワークスペースの `.vscode` にマウントされます。

プロジェクトに `.vscode` ディレクトリが存在する場合、その内容はサンドボックス側の `.vscode` にコピーされます。ただし、`tasks.json` は ai-cage の `autoStartCommand` 用タスクで上書きされます。元から `.vscode/tasks.json` が存在していた場合は、起動時に警告が表示されます。
