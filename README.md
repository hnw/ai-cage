...
| `autoStartAgent` | `string \| null` | VS Code 起動時に自動的に起動するエージェント |
| `image` | `string` | DevContainer のベースイメージ |
| `mountAuth` | `boolean` | ホスト側の認証ディレクトリをマウントするか |

## エージェントの自動起動

設定ファイルで `autoStartAgent` を指定すると、生成される DevContainer 内に `.vscode/tasks.json` が作成され、フォルダを開いたタイミングで指定のエージェントを統合ターミナル上で自動起動します。
